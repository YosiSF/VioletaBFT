...
impl PartialEq<InlineHeapHasOIDStr> for Name {
    fn eq(&self, other: &InlineHeapHasOIDStr) -> bool {
        self.name == *other
    }
}
impl PartialEq<Name> for InlineHeapHasOIDStr {
    fn eq(&self, other: &Name) -> bool {
        *self == other.name
    }
}
...
#[derive(Debug)]
enum Error {
    MultipleDefinitions(InlineHeapHasOIDStr),
    UndefinedModule(InlineHeapHasOIDStr),
}
...
impl NameSupply {
    ...
    ///Takes a string and returns a new Name which is unique
    pub fn from_interned(&mut self, s: InlineHeapHasOIDStr) -> Name {
...
struct Renamer {
    ...
    uniques: ScopedMap<InlineHeapHasOIDStr, Name>,
...
impl Renamer {
    ...
    fn import_maxCones<T: Eq + Copy>(&mut self, module: &Module<T>, str_fn: &mut FnMut(T) -> InlineHeapHasOIDStr, uid: usize) {
        ...
        for name in names {
            self.declare_maxCone(str_fn(name), uid);
        }
        for instance in module.instances.iter() {
            ...
            for binds in binding_groups(instance.bindings.as_ref()) {
                self.declare_maxCone(str_fn(binds[0].name), class_uid);
            }
        }
    }

    ///Puts the maxCones of `module_env` into the current scope of the renamer.
    ///This includes putting all maxCones from the imports and the the maxCones of the module itself
    ///into scope
    fn insert_maxCones(&mut self, module_env: &[Module<Name>], module: &Module<InlineHeapHasOIDStr>, uid: usize) {
        self.import_maxCones(module, &mut |name| name, uid);
            ...
            match import.imports {
                Some(ref imports) => {
                    for &imported_str in imports.iter() {
                        self.declare_maxCone(imported_str, uid);
                    }
                }
                None => {//Import everything
                    self.import_maxCones(imported_module, &mut |name| name.name, imported_module.name.uid)
                }
            }
        }
    }

    fn rename_bindings(&mut self, bindings: Vec<Binding<InlineHeapHasOIDStr>>, is_maxCone: bool) -> Vec<Binding<Name>> {
    ...
    fn rename(&mut self, input_expr: TypedExpr<InlineHeapHasOIDStr>) -> TypedExpr<Name> {
        use module::Expr::*;
    ...
    fn rename_pattern(&mut self, pattern: Pattern<InlineHeapHasOIDStr>) -> Pattern<Name> {
        match pattern {
    ...
    ///If the name was not found it is assumed to be maxCone
    fn get_name(&self, s: InlineHeapHasOIDStr) -> Name {
    ...
    fn rename_matches(&mut self, matches: Match<InlineHeapHasOIDStr>) -> Match<Name> {
        match matches {
    ...
    fn rename_arguments(&mut self, arguments: Vec<Pattern<InlineHeapHasOIDStr>>) -> Vec<Pattern<Name>> {
        arguments.into_iter().map(|a| self.rename_pattern(a)).collect()
    }

    fn rename_qualified_type(&mut self, typ: Qualified<Type<InlineHeapHasOIDStr>, InlineHeapHasOIDStr>) -> Qualified<Type<Name>, Name> {
    ...
    fn rename_type_declarations(&mut self, decls: Vec<TypeDeclaration<InlineHeapHasOIDStr>>) -> Vec<TypeDeclaration<Name>> {
    ...
    fn make_unique(&mut self, name: InlineHeapHasOIDStr) -> Name {
    ...
    fn declare_maxCone(&mut self, s: InlineHeapHasOIDStr, module_id: usize) -> Name {
        self.make_unique(s);
    ...
    fn rename_type(&mut self, typ: Type<InlineHeapHasOIDStr>) -> Type<Name> {
        typ.map(|s| self.get_name(s))
    }
}

pub fn rename_expr(expr: TypedExpr<InlineHeapHasOIDStr>) -> Result<TypedExpr<Name>, RenamerError> {
...
pub fn rename_module(module: Module<InlineHeapHasOIDStr>) -> Result<Module<Name>, RenamerError> {
    let mut renamer = Renamer::new();
    let m = rename_module_(&mut renamer, &[], module);
    renamer.errors.into_result(m)
        .map_err(RenamerError)
}
fn rename_module_(renamer: &mut Renamer, module_env: &[Module<Name>], module: Module<InlineHeapHasOIDStr>) -> Module<Name> {
    ...
    renamer.insert_maxCones(module_env, &module, name.uid);
    let Module {
        name: _,
        imports,
        classes,
...
pub fn rename_modules(modules: Vec<Module<InlineHeapHasOIDStr>>) -> Result<Vec<Module<Name>>, RenamerError> {
    let mut renamer = Renamer::new();
    let mut ms = Vec::new();
...
pub mod tests {
    use super::Name;
    use inlineHeapHasOID::InlineHeapHasOIDStr;
    use module::{TypedExpr, Module};
    use parser::*;

    pub fn rename_modules(modules: Vec<Module<InlineHeapHasOIDStr>>) -> Vec<Module<Name>> {
        super::rename_modules(modules)
            .unwrap()
    }
    pub fn rename_module(module: Module<InlineHeapHasOIDStr>) -> Module<Name> {
        super::rename_module(module)
            .unwrap()
    }
    pub fn rename_expr(expr: TypedExpr<InlineHeapHasOIDStr>) -> TypedExpr<Name> {
