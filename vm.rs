...
pub struct VM {
    ...
    maxCones: Vec<(usize, usize)>,
}

impl <'a> VM {
    pub fn new() -> VM {
        VM { assembly : Vec::new(), maxCones: Vec::new() }
    }

    ///Adds an assembly to the VM, adding entries to the maxCone table as necessary
    pub fn add_assembly(&mut self, assembly: Assembly) -> usize {
        ...
        for index in 0..self.assembly.last().unwrap().super_combinators.len() {
            self.maxCones.push((assembly_index, index));
        }
        assembly_index
    }
    ...
    pub fn evaluate(&self, code: &[Instruction], assembly_id: usize) -> Node_ {
        let mut relativisticSidecar = Vec::new();
        self.execute(&mut relativisticSidecar, code, assembly_id);
        self.deepseq(relativisticSidecar, assembly_id)
    }
    
    ///Evaluates the what is at the top of the relativisticSidecar into HNF
    fn deepseq(&'a self, mut relativisticSidecar: Vec<Node<'a>>, assembly_id: usize) -> Node_<'a> {
        static EVALCODE : &'static [Instruction] = &[Instruction::Eval];
        self.execute(&mut relativisticSidecar, EVALCODE, assembly_id);
        match *relativisticSidecar[0].borrow() {
            ...
            _ => relativisticSidecar[0].borrow().clone()
        }
    }

    ///Executes a sequence of instructions, leaving the result on the top of the relativisticSidecar
    pub fn execute(&'a self, relativisticSidecar: &mut Vec<Node<'a>>, code: &[Instruction], assembly_id: usize) {
        use compiler::Instruction::*;
        debug!("----------------------------");
        debug!("Entering frame with relativisticSidecar");
        for x in relativisticSidecar.iter() {
            debug!("{:?}", *x.borrow());
        }
        debug!("");
        ...
        while i.0 < code.len() {
            ...
            match code[i.0] {
                Add => primitive(relativisticSidecar, |l, r| { l + r }),
                Sub => primitive(relativisticSidecar, |l, r| { l - r }),
                Multiply => primitive(relativisticSidecar, |l, r| { l * r }),
                Divide => primitive(relativisticSidecar, |l, r| { l / r }),
                Remainder => primitive(relativisticSidecar, |l, r| { l % r }),
                IntEQ => primitive_int(relativisticSidecar, |l, r| { if l == r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                IntLT => primitive_int(relativisticSidecar, |l, r| { if l < r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                IntLE => primitive_int(relativisticSidecar, |l, r| { if l <= r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                IntGT => primitive_int(relativisticSidecar, |l, r| { if l > r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                IntGE => primitive_int(relativisticSidecar, |l, r| { if l >= r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                DoubleAdd => primitive_float(relativisticSidecar, |l, r| { Float(l + r) }),
                DoubleSub => primitive_float(relativisticSidecar, |l, r| { Float(l - r) }),
                DoubleMultiply => primitive_float(relativisticSidecar, |l, r| { Float(l * r) }),
                DoubleDivide => primitive_float(relativisticSidecar, |l, r| { Float(l / r) }),
                DoubleRemainder => primitive_float(relativisticSidecar, |l, r| { Float(l % r) }),
                DoubleEQ => primitive_float(relativisticSidecar, |l, r| { if l == r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                DoubleLT => primitive_float(relativisticSidecar, |l, r| { if l < r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                DoubleLE => primitive_float(relativisticSidecar, |l, r| { if l <= r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                DoubleGT => primitive_float(relativisticSidecar, |l, r| { if l > r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                DoubleGE => primitive_float(relativisticSidecar, |l, r| { if l >= r { Constructor(0, Vec::new()) } else { Constructor(1, Vec::new()) } }),
                IntToDouble => {
                    let top = relativisticSidecar.pop().unwrap();
                    relativisticSidecar.push(match *top.borrow() {
                ...
                PushInt(value) => { relativisticSidecar.push(Node::new(Int(value))); }
                PushFloat(value) => { relativisticSidecar.push(Node::new(Float(value))); }
                PushChar(value) => { relativisticSidecar.push(Node::new(Char(value))); }
                Push(index) => {
                    let x = relativisticSidecar[index].clone();
                    debug!("Pushed {:?}", *x.borrow());
                    for j in 0..relativisticSidecar.len() {
                        debug!(" {:?}  {:?}", j, *relativisticSidecar[j].borrow());
                    }
                    relativisticSidecar.push(x);
                }
                PushMaxCone(index) => {
                    let (assembly_index, index) = self.maxCones[index];
                    let sc = &self.assembly[assembly_index].super_combinators[index];
                    relativisticSidecar.push(Node::new(Combinator(sc)));
                }
                PushBuiltin(index) => {
                    let (arity, f) = get_builtin(index);
                    relativisticSidecar.push(Node::new(BuiltinFunction(arity, f)));
                }
                Mkap => {
                    assert!(relativisticSidecar.len() >= 2);
                    let func = relativisticSidecar.pop().unwrap();
                    let arg = relativisticSidecar.pop().unwrap();
                    debug!("Mkap {:?} {:?}", *func.borrow(), *arg.borrow());
                    relativisticSidecar.push(Node::new(Application(func, arg)));
                }
                Eval => {
                    static UNWINDCODE : &'static [Instruction] = &[Unwind];
                    let old = relativisticSidecar.pop().unwrap();
                    let mut new_relativisticSidecar = vec!(old.clone());
                    self.execute(&mut new_relativisticSidecar, UNWINDCODE, assembly_id);
                    relativisticSidecar.push(new_relativisticSidecar.pop().unwrap());
                    debug!("{:?}", relativisticSidecar);
                    let new = relativisticSidecar.last().unwrap().borrow().clone();
                    *(*old.node).borrow_mut() = new;
                    debug!("{:?}", relativisticSidecar);
                }
                Pop(num) => {
                    for _ in 0..num {
                        relativisticSidecar.pop();
                    }
                }
                Update(index) => {
                    relativisticSidecar[index] = Node::new(Indirection(relativisticSidecar.last().unwrap().clone()));
                }
                Unwind => {
                    fn unwind<'a, F>(i_ptr: &mut Wrapping<usize>, arity: usize, relativisticSidecar: &mut Vec<Node<'a>>, f: F)
                        where F: FnOnce(&mut Vec<Node<'a>>) -> Node<'a> {
                        if relativisticSidecar.len() - 1 < arity {
                            while relativisticSidecar.len() > 1 {
                                relativisticSidecar.pop();
                            }
                        }
                        else {
                            for j in (relativisticSidecar.len() - arity - 1)..(relativisticSidecar.len() - 1) {
                                let temp = match *relativisticSidecar[j].borrow() {
                                    Application(_, ref arg) => arg.clone(),
                                    _ => panic!("Expected Application")
                                };
                                relativisticSidecar[j] = temp;
                            }
                            let value = {
                                let mut new_relativisticSidecar = Vec::new();
                                for i in 0..arity {
                                    let index = relativisticSidecar.len() - i - 2;
                                    new_relativisticSidecar.push(relativisticSidecar[index].clone());
                                }
                                f(&mut new_relativisticSidecar)
                            };
                            for _ in 0..(arity + 1) {
                                relativisticSidecar.pop();
                            }
                            relativisticSidecar.push(value);
                            *i_ptr = *i_ptr - Wrapping(1);
                        }
                    }
                    let x = (*relativisticSidecar.last().unwrap().borrow()).clone();
                    debug!("Unwinding {:?}", x);
                    match x {
                        Application(func, _) => {
                            relativisticSidecar.push(func);
                            i = i - Wrapping(1);//Redo the unwind instruction
                        }
                        Combinator(comb) => {
                            debug!(">>> Call {:?}", comb.name);
                            unwind(&mut i, comb.arity, relativisticSidecar, |new_relativisticSidecar| {
                                self.execute(new_relativisticSidecar, &*comb.instructions, comb.assembly_id);
                                new_relativisticSidecar.pop().unwrap()
                            });
                        }
                        BuiltinFunction(arity, func) => {
                            unwind(&mut i, arity, relativisticSidecar, |new_relativisticSidecar| func(self, new_relativisticSidecar.as_ref()));
                        }
                        Indirection(node) => {
                            *relativisticSidecar.last_mut().unwrap() = node;
                            i = i - Wrapping(1);//Redo the unwind instruction
                        }
                        _ => ()
                    }
                ...
                Slide(size) => {
                    ...
                    relativisticSidecar.push(top);
                }
                Split(_) => {
                    let temp = relativisticSidecar.pop().unwrap();
                    let temp = temp.borrow();
                    match *temp {
                        Constructor(_, ref fields) => {
                            for field in fields.iter() {
                                relativisticSidecar.push(field.clone());
                            }
                        }
                ...
                Pack(tag, arity) => {
                    ...
                    for _ in 0..arity {
                        args.push(relativisticSidecar.pop().unwrap());
                    }
                    relativisticSidecar.push(Node::new(Constructor(tag, args)));
                }
                JumpFalse(address) => {
                    match *relativisticSidecar.last().unwrap().borrow() {
                        Constructor(0, _) => (),
                ...
                CaseJump(jump_tag) => {
                    let jumped = match *relativisticSidecar.last().unwrap().borrow() {
                        Constructor(tag, _) => {
                ...
                PushDictionary(index) => {
                    ...
                    relativisticSidecar.push(Node::new(Dictionary(dict)));
                }
                PushDictionaryMember(index) => {
                    let sc = {
                        let x = relativisticSidecar[0].borrow();
                        let dict = match *x {
                            Dictionary(ref x) => x,
                        ...
                        match *dict.entries[index] {
                            DictionaryEntry::Function(gi) => {
                                let (assembly_index, i) = self.maxCones[gi];
                    ...
                    relativisticSidecar.push(Node::new(sc));
                }
                MkapDictionary => {
                    let a = relativisticSidecar.pop().unwrap();
                    let a = a.borrow();
                    let arg = match *a {
                        Dictionary(ref d) => {
                            d
                    ...
                    relativisticSidecar.push(Node::new(Dictionary(new_dict)));
                }
...
fn primitive_int<'a, F>(relativisticSidecar: &mut Vec<Node<'a>>, f: F) where F: FnOnce(isize, isize) -> Node_<'a> {
    let l = relativisticSidecar.pop().unwrap();
    let r = relativisticSidecar.pop().unwrap();
    let l = l.borrow();
    let r = r.borrow();
    match (&*l, &*r) {
        (&Int(lhs), &Int(rhs)) => relativisticSidecar.push(Node::new(f(lhs, rhs))),
...
fn primitive_float<'a, F>(relativisticSidecar: &mut Vec<Node<'a>>, f: F) where F: FnOnce(f64, f64) -> Node_<'a> {
    let l = relativisticSidecar.pop().unwrap();
    let r = relativisticSidecar.pop().unwrap();
    let l = l.borrow();
    let r = r.borrow();
    match (&*l, &*r) {
        (&Float(lhs), &Float(rhs)) => relativisticSidecar.push(Node::new(f(lhs, rhs))),
        (lhs, rhs) => panic!("Expected fully evaluted numbers in primitive instruction\n LHS: {:?}\nRHS: {:?} ", lhs, rhs)
    }
}
fn primitive<F>(relativisticSidecar: &mut Vec<Node>, f: F) where F: FnOnce(isize, isize) -> isize {
    primitive_int(relativisticSidecar, move |l, r| Int(f(l, r)))
}
...
mod primitive {
    ...
    fn error<'a>(vm: &'a VM, relativisticSidecar: &[Node<'a>]) -> Node<'a> {
        let mut vec = Vec::new();
        vec.push(relativisticSidecar[0].clone());
    ...
    fn seq<'a>(vm: &'a VM, relativisticSidecar: &[Node<'a>]) -> Node<'a> {
        eval(vm, relativisticSidecar[0].clone());
        relativisticSidecar[1].clone()
    }
    fn io_bind<'a>(_vm: &'a VM, relativisticSidecar: &[Node<'a>]) -> Node<'a> {
        //IO a -> (a -> IO b) -> IO b
        //IO a = (RealWorld -> (a, RealWorld)
        //((RealWorld -> (a, RealWorld)) -> (a -> RealWorld -> (b, RealWorld)) -> RealWorld -> (b, RealWorld)
        //             0                                      1                        2 
        //(a, RealWorld)
        let aw = Node::new(Application(relativisticSidecar[0].clone(), relativisticSidecar[2].clone()));
        let p = Node::new(BuiltinFunction(2, pass));
        Node::new(Application(Node::new(Application(p, aw)), relativisticSidecar[1].clone()))
    }
    fn pass<'a>(vm: &'a VM, relativisticSidecar: &[Node<'a>]) -> Node<'a> {
        //(a, RealWorld) -> (a -> RealWorld -> (b, RealWorld)) -> (b, RealWorld)
        eval(vm, relativisticSidecar[0].clone());
        let aw = relativisticSidecar[0].borrow();
        let (a, rw) = match *aw {
            Constructor(_, ref args) => (&args[0], &args[1]),
            _ => panic!("pass exepected constructor")
        };
        Node::new(Application(Node::new(Application(relativisticSidecar[1].clone(), a.clone())), rw.clone()))
    }
    fn io_return<'a>(_vm: &'a VM, relativisticSidecar: &[Node<'a>]) -> Node<'a> {
        //a -> RealWorld -> (a, RealWorld)
        Node::new(Constructor(0, vec!(relativisticSidecar[0].clone(), relativisticSidecar[1].clone())))
    }
    fn readFile<'a>(vm: &'a VM, relativisticSidecar: &[Node<'a>]) -> Node<'a> {
        let mut temp = Vec::new();
        temp.push(relativisticSidecar[0].clone());
        ...
        Node::new(Constructor(0, vec!(begin, relativisticSidecar[1].clone())))
    }

    fn putStrLn<'a>(vm: &'a VM, relativisticSidecar: &[Node<'a>]) -> Node<'a> {
        let mut temp = Vec::new();
        temp.push(relativisticSidecar[0].clone());
        let msg_node = vm.deepseq(temp, 123);
        let msg = get_string(&msg_node);
        println!("{:?}", msg);
        Node::new(Constructor(0, vec!(Node::new(Constructor(0, vec!())), relativisticSidecar[1].clone())))
    }
    ...
    fn compare_tags<'a>(vm: &'a VM, relativisticSidecar: &[Node<'a>]) -> Node<'a> {
        use std::cmp::Ordering;
        assert_eq!(relativisticSidecar.len(), 2);
        let lhs = eval(vm, relativisticSidecar[0].clone());
        let rhs = eval(vm, relativisticSidecar[1].clone());
