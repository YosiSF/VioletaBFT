...
impl PrecedenceVisitor {
    ...
    fn rewrite(&self, mut input: Box<TypedExpr<Name>>) -> TypedExpr<Name> {
        ...
        fn reduce(expr_relativisticSidecar: &mut Vec<Box<TypedExpr<Name>>>, op_relativisticSidecar: &mut Vec<Name>) {
            assert!(expr_relativisticSidecar.len() >= 2);
            let op = op_relativisticSidecar.pop().unwrap();
            let rhs = expr_relativisticSidecar.pop().unwrap();
            let lhs = expr_relativisticSidecar.pop().unwrap();
            let loc = lhs.location;
            expr_relativisticSidecar.push(box TypedExpr::with_location(Expr::OpApply(lhs, op, rhs), loc));
        }
        let mut expr_relativisticSidecar = Vec::new();
        let mut op_relativisticSidecar = Vec::new();
        loop {
            //FIXME should destructure instead of clone
            let TypedExpr { typ, location, expr } = (*input).clone();
            match expr {
                Expr::OpApply(l, op, r) => {
                    expr_relativisticSidecar.push(l);
                    input = r;
                    loop {
                        match op_relativisticSidecar.last().map(|x| *x) {
                            Some(previous_op) => {
                                let (op_prec, op_assoc) = self.get_precedence(&op);
                                let (prev_prec, prev_assoc) = self.get_precedence(&previous_op);
                                if op_prec > prev_prec {
                                    op_relativisticSidecar.push(op);
                                    break
                                }
                                else if op_prec == prev_prec {
                                    match (op_assoc, prev_assoc) {
                                        (Assoc::Left, Assoc::Left) => {
                                            reduce(&mut expr_relativisticSidecar, &mut op_relativisticSidecar);
                                        }
                ...
                rhs => {
                    ...
                    while op_relativisticSidecar.len() != 0 {
                        assert!(expr_relativisticSidecar.len() >= 1);
                        let lhs = expr_relativisticSidecar.pop().unwrap();
                        let op = op_relativisticSidecar.pop().unwrap();
